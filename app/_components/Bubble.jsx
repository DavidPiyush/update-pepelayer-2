...
function Bubble() {
  ...
  useEffect(() => {
    async function fetchUserData() {
      if (ethereumId) {
        ...
        setDailyClaim(res.todayClaim);
        setTimerStart(res.timerStart); // Fetch timerStart from the database
        console.log(res.timerStart, "this from database");
      }
    }
  ...
  async function handleClick(e) {
    ...
    try {
      ...
      const updateData = {
        todayClaim: reward, // Update today's claim amount
      };

      if (!timerStart || timeExpired) {
        // If no timer or timer expired, allow claim and reset the timer
        updateData.timerStart = new Date(); // Start the timer when button is clicked
        updateData.timerExpired = false; // Reset the expired flag
